...
static bool memoryMonitor(void* userPtr, ssize_t bytes, bool post)
{
    return true;
}
...
struct InnerNode : Node {
    ...
    InnerNode()
    {
        bounds[0] = bounds[1] = bounds[2] = bounds[3] = BoundingBox{};
                    ...
                    bool regenerate, bool cache)
{
    ...
    for(int mesh_idx = 0; mesh_idx < object.meshes.size(); mesh_idx++) {
        ...
        for(int face_idx = 0; face_idx < mesh.numFaces; face_idx++) {
    ...
    madrona::Optional<std::ofstream> out =
        madrona::Optional<std::ofstream>::none();
    ...
    if (innerID == 0) {
        madrona::phys::MeshBVH::Node node;
...
//#define COMPRESSED
    ...
    for(int i=0;i<leafID;i++){
        ...
        madrona::phys::MeshBVH::LeafGeometry geos;
    ...
    for(int i=0;i<leafID;i++){
        LeafNode* node = leafNodes[i];
        madrona::phys::MeshBVH::LeafMaterial geos;
        for(int i2=0;i2<numTrisPerLeaf;i2++){
